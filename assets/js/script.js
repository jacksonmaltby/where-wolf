...
searchButton.addEventListener("click", async () => {
  ...
  const response = await fetch(`https://cors-anywhere.herokuapp.com/https://api.yelp.com/v3/businesses/search?term=${term}&location=${location}`, {
    headers: {
  ...
  data.businesses.forEach(business => {
    ...
    li.innerHTML = `<h3>${business.name}</h3><p>${business.location.address}</p><p>${business.location.city}</p><p>${business.location.state}</p><p>${business.location.zip_code}</p><p>${category.value}</p>`;
  ...
  map.fitBounds(bounds);
    ...
    marker.bindPopup(`<h3>${business.name}</h3><p>${business.location.address}</p><p>${category.value}</p>`);
  });
});